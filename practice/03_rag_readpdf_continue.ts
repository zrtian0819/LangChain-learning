/**
 * RAG 範例：讀取 PDF 文件並建立向量資料庫，支援在終端機連續提問
 *
 * 工作流程：
 ...
 * 6. 支援連續提問，輸入 "exit" 或 "quit" 結束
 *
 * 執行指令: bun run practice/03_rag_readpdf_continue.ts
 */
...
async function runRAG() {
  try {
    ...
    const askQuestion = () => {
      rl.question('\n💬 請輸入你的問題（輸入 "exit" 結束）：', async (question: string) => {
    ...
    console.log('🚀 PDF 已準備完成！你可以開始提問（輸入 "exit" 結束）\n');
    askQuestion();
  } catch (error) {
